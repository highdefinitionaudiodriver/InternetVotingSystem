...
### 5.1 上書き投票（強要対策）
- 投票期間中、同一voter_hashから複数回の投票券発行を許容する（revote_count++）。このため、`voter_status.token_issued` は「過去に1回以上発行済み」を示す監査用フラグであり、再発行禁止フラグではない。
- 同一投票券トークンで投票内容を上書きする場合、投票受付サーバーは同一blind_token_hashの票をrevote_serialを増やしながら受理する。
- 新しい投票券トークンを再発行した場合は、発行サーバーの `last_issued_token_hash` を更新し、集計直前の前処理で同一voter_hash由来の最新発行トークンのみを有効化する。ただし匿名投票ゾーンにvoter_hashを渡してはならないため、この対応表は認証ゾーン内で完結し、ミックスネット投入前にトークン有効/失効リストとしてのみ反映する。
- 集計直前のミックスネット投入前段で、各有効トークンにつき最大revote_serialの票のみを採用する。
...
### 8.5 現行プロトタイプの実装範囲
本リポジトリの初期実装は、制度・データフロー・API境界を確認するためのローカルプロトタイプである。外部依存を増やさず標準ライブラリ中心で動作させるため、暗号処理は `DemoCryptoSuite` に隔離し、HMAC/SHAベースの検証可能なデモ処理として実装している。本番化時はこの境界を、RFC 9474準拠ブラインド署名、ElGamal/ristretto255、Chaum-Pedersen系ZKP、DKG、Shamir閾値復号に置き換えること。
